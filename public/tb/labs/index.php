...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
  ...
  <div class="lg:col-span-1 bg-white p-5 rounded shadow h-fit">
    ...
    <form method="POST" class="space-y-3">
      <div>
        ...
        <select name="tb_case_id" required class="w-full border rounded px-3 py-2 text-sm">
      ...
      <div>
        ...
        <input type="date" name="test_date" value="<?= date('Y-m-d') ?>" required class="w-full border rounded px-3 py-2 text-sm" />
      </div>

      <div>
        <label class="block text-xs font-medium text-slate-700 mb-1">Test Type *</label>
        <select name="test_type" required class="w-full border rounded px-3 py-2 text-sm">
      ...
      <div>
        ...
        <select name="timing" required class="w-full border rounded px-3 py-2 text-sm">
      ...
      <div>
        ...
        <input type="text" name="result" placeholder="e.g. Negative, 1+, 2+, MTB Detected" required class="w-full border rounded px-3 py-2 text-sm" />
      </div>

      <div>
        <label class="block text-xs font-medium text-slate-700 mb-1">Testing Laboratory</label>
        <input type="text" name="laboratory_name" placeholder="e.g. Health Center Lab" class="w-full border rounded px-3 py-2 text-sm" />
      </div>

      <div>
        <label class="block text-xs font-medium text-slate-700 mb-1">Notes</label>
        <textarea name="notes" rows="2" placeholder="Details or resistance info..." class="w-full border rounded px-3 py-2 text-sm"></textarea>
      </div>

      <button type="submit" class="w-full bg-slate-900 text-white py-2 rounded text-sm font-medium hover:bg-slate-800 transition">Save Lab Result</button>
    </form>
  </div>
  ...
  <div class="lg:col-span-2 bg-white p-5 rounded shadow">
    <div class="flex items-center justify-between">
      <div>
        <text class="text-sm text-slate-500">Log History</text>
          ...
          <tbody class="divide-y text-slate-700">
            <?php foreach ($labTests as $lt): ?>
              <tr>
                ...
                <td class="py-3 px-3 uppercase text-xs font-semibold text-slate-800"><?= h(str_replace('_', ' ', $lt['test_type'])) ?></td>
                <td class="py-3 px-3 capitalize text-xs"><?= h(str_replace('_', ' ', $lt['timing'])) ?></td>
                <td class="py-3 px-3 font-medium text-slate-900">
                  <?= h($lt['result']) ?>
                ...
                <td class="py-3 px-3 text-xs text-slate-500"><?= h($lt['laboratory_name'] ?? '—') ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
