...
<div class="bg-white p-6 rounded shadow">
  <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
    ...
    <div class="flex items-center gap-2">
      <a href="/HealthLogs/public/tb/cases/index.php" class="text-sm text-slate-600 underline">Back to Registry</a>
    </div>
  </div>
</div>
...
<form method="POST" class="mt-6 bg-white rounded shadow p-6 space-y-4">
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <!-- Select Patient -->
    <div class="md:col-span-2">
      <label class="block text-sm font-medium text-slate-700 mb-1">Select Patient *</label>
      <select name="patient_id" required class="w-full border rounded px-3 py-2 text-sm">
    ...
    <div>
      ...
      <input type="text" name="case_number" value="<?= h($_POST['case_number'] ?? $nextCaseNo) ?>" required class="w-full border rounded px-3 py-2 text-sm font-mono" />
    </div>

    <!-- Registration Date -->
    <div>
      <label class="block text-sm font-medium text-slate-700 mb-1">Registration Date *</label>
      <input type="date" name="registration_date" value="<?= h($_POST['registration_date'] ?? date('Y-m-d')) ?>" required class="w-full border rounded px-3 py-2 text-sm" />
    </div>

    <!-- Anatomical Site / TB Type -->
    <div>
      <label class="block text-sm font-medium text-slate-700 mb-1">Anatomical Site / TB Type *</label>
      <select name="tb_type" required class="w-full border rounded px-3 py-2 text-sm">
    ...
    <div>
      ...
      <select name="case_definition" required class="w-full border rounded px-3 py-2 text-sm">
    ...
    <div>
      ...
      <select name="bacteriological_status" required class="w-full border rounded px-3 py-2 text-sm">
    ...
    <div>
      ...
      <select name="treatment_category" required class="w-full border rounded px-3 py-2 text-sm">
    ...
    <div>
      ...
      <input type="date" name="treatment_start_date" value="<?= h($_POST['treatment_start_date'] ?? date('Y-m-d')) ?>" required class="w-full border rounded px-3 py-2 text-sm" />
    </div>

    <!-- Clinical Notes -->
    <div class="md:col-span-2">
      <label class="block text-sm font-medium text-slate-700 mb-1">Clinical Notes & Remarks</label>
      <textarea name="notes" rows="3" class="w-full border rounded px-3 py-2 text-sm"><?= h($_POST['notes'] ?? '') ?></textarea>
    </div>
  </div>

  <div class="flex items-center gap-3 pt-4 border-t">
    <button type="submit" class="bg-slate-900 text-white px-4 py-2 rounded text-sm font-medium">Save & Register Case</button>
    <a href="/HealthLogs/public/tb/cases/index.php" class="px-4 py-2 rounded border border-slate-300 text-slate-700 text-sm">Cancel</a>
  </div>
</form>
