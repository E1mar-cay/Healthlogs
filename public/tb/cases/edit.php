...
<form method="POST" class="mt-6 bg-white rounded shadow p-6 space-y-4">
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
      ...
      <input type="date" name="treatment_start_date" value="<?= h($case['treatment_start_date']) ?>" required class="w-full border rounded px-3 py-2 text-sm" />
    </div>
    ...
    <div>
      ...
      <select name="treatment_outcome" class="w-full border rounded px-3 py-2 text-sm">
    ...
    <div>
      ...
      <input type="date" name="treatment_outcome_date" value="<?= h($case['treatment_outcome_date'] ?? '') ?>" class="w-full border rounded px-3 py-2 text-sm" />
    </div>

    <!-- Clinical Notes -->
    <div class="md:col-span-2">
      <label class="block text-sm font-medium text-slate-700 mb-1">Clinical Notes & Progress</label>
      <textarea name="notes" rows="3" class="w-full border rounded px-3 py-2 text-sm"><?= h($case['notes'] ?? '') ?></textarea>
    </div>
  </div>

  <div class="flex items-center gap-3 pt-4 border-t">
    <button type="submit" class="bg-slate-900 text-white px-4 py-2 rounded text-sm font-medium">Update Case Details</button>
